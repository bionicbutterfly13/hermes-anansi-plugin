...
**Phase:** 1 — Skeleton + State ✓ complete → Phase 2 — Appraisal Path
**Status:** verifying (16/17; one cosmetic network-blocked re-run)
...
- **Language:** Python 3.11 (matches hermes-agent venv at `~/.hermes/hermes-agent/venv`)
...
- **Tests:** `~/.hermes/hermes-agent/venv/bin/python -m pytest tests/` (host ships `make_plugin_llm_for_test()` helper)
