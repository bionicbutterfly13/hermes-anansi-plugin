...
def test_v2_db_quarantined_and_recreated_at_v3(tmp_path):
    ...
    # A v2 DB is non-additive from the current schema, so it keeps recreating at
    # the CURRENT version — the v3 assistant_excerpt column is still part of it.
    # Assert against store.SCHEMA_VERSION so the disposable-state proof survives
    # future bumps.
